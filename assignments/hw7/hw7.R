######################################################
##### Homework 7 due Friday April 17 by midnight
...
# Check thes class and dimension of [speeches].  Open the textfile in 
# an editor and compare it to [speeches]

speeches <- readLines(con=file("stateoftheunion1790-2012.txt"))
readLines("stateoftheunion1790-2012.txt")
...
speechlines <- strsplit(onespeech, "\\*\\*\\*")
for(i in 1:n.speeches){
  speechesL[i] <- speechlines[[1]][i+1]
...
# Output : words, a character vector where each element is one word 
  ...
  lowercase <- tolower(sentences)
  noapostrophe <- gsub("[[:apostrophe:]]", "", lowercase)
  nonum <- gsub("[[:digit:]]", "", noclaps)
    
  # Drop the words (Applause. and Laughter.)
  noapplause <- gsub("Applause", "", nopunct)
  nolaughter <- gsub("Laughter", "", nopunct)
...
everyword <- c(0)
for(i in 1:n.speeches) {
  allwords = c(everyword, speechWords[[i]])

}

uniqueWords <- sort(unique(everyword))
...
count <- list(0)
for(i in 1:222) {count[[i]] <- table(speechToWords(speechlines[[1]][i+1])) }

for(j in 1:222) {
  for(i in 1:length(uniqueWords)) {
    wordVecs[i+(j-1)*length(uniqueWords)] <- 
      max(count[[j]]
...
Pres <- speechesDF$Pres
initial <- speechesDF$initial
party <- speechesDF$party
...
text(mds, labels = unique(speechesDF$initial))
