...
name <- "Sriharsha Rao Kondapally"
github.name <- "SriKondapally"
email.address <- "hensels@berkeley.edu"
...
cw3 <- ChickWeight[ChickWeight$weight== mean(ChickWeight$Weight) & ChickWeight$Time== 0,]
...
carType <- lm(mtcars$mpg ~ mtcars$cyl)
plot(x= mtcars$mpg, y= mtcars$cyl, xlab= "mpg", ylab= "cyl")
abline(coefficients(carType[1]), coefficients(carType[2]))
...
colors <- rep("blue", nrow(mtcars))
colors[mtcars$am == 0] = "red"
plot(x= mtcars$wt, y= mtcars$disp, col= colors, xlab= "weight (lb/1000)" , 
     ylab= "cubic inches displacement")
...
plot(x= WorldBank$life.expectancy, y= WorldBank$fertility.rate, col= WorldBank$region,
...
par(mfrow= c(1,2))
plot(x= WorldBank$life.expectancy[WorldBank$year== 1960], y= WorldBank$fertility.rate[WorldBank$year== 1960], 
     col= WorldBank$region, xlab= "Life Expectancy", ylab= "Fertility")
plot(x= WorldBank$life.expectancy[WorldBank$year== 2013], y= WorldBank$fertility.rate[WorldBank$year== 2013], 
     col= WorldBank$region, xlab= "Life Expectancy", ylab= "Fertility")
...
PermDiff <- function(cases, controls, k=5000){
  caselen = length(cases)
  controllen = length(controls)
  combined = caselen + controllen
  
  for(i in 1:k){
    conf = c(cases, controls)
    shuffle = sapply(conf, combined)
    cases = shuffle[1: cases.ms]
    controls = mean(cases) - mean(controls) 
    res = c(rec, mean(cases), mean(controls))
  }
  return(res)
}
...
NumJackpot <- function(k, B){
  ...
  for(i in 1:B){
    ...
    winners = 0
    for(i in 1:k){
      ticket = sample(1:19, 3, replace = FALSE)
      if(all(sort(winning)) == sort(ticket))
        winners = winners + 1
    }
    res[i] = winners 
  }
...
text4 <- sapply(phrases, function(item)substr(item,1,3))
